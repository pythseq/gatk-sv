...
workflow MakeBincovMatrix {
  ...
  if(defined(bincov_matrix_samples)) {
    String bincov_matrix_header = "~{sep='\t' bincov_matrix_samples}"
  }
  ...
  call ZPaste {
    input:
      ...
      matrix_file_name = "~{batch}.bincov.bed.gz",
      disk_overhead_gb = disk_overhead_gb,
...
task MakeBincovMatrixColumns {
  ...
  String bincov_file_name = "~{basename(count_file, '.tsv.gz')}.bincov.bed.gz"
...
task ZPaste {
  ...
  # This method works and is faster with localization_optional, but for large batches the gsutil cp loses connection.
  # For safety sake, turn off for now. Could be turned back on if
  #  a) gsutil becomes more robust
  #  b) localization_optional becomes switchable, in which case it can be toggled on for small batch sizes
  parameter_meta {
    column_files: {
  ...
  command <<<
    ...
    while read -r COLUMN_FILE; do
      ...
      mkfifo "$FIFO"
      if [[ "$COLUMN_FILE" == gs://* ]]; then
        # the file has not been localized
        gsutil cp "$COLUMN_FILE" - | bgzip -cd > "$FIFO" &
      else
