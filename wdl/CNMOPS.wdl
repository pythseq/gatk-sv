...
task CNSampleNormal {
  ...
  Int disk_gb = disk_gb_base + ceil(size([black, ped, bincov_matrix_index], "GiB") + disk_gb_scale * size(bincov_matrix, "GiB"))
  RuntimeAttr default_attr = object {
    cpu_cores: 1, 
    mem_gb: mem_gb,
    disk_gb: disk_gb,
  ...
  command <<<
    ...
    set +e
    bash /opt/WGD/bin/cnMOPS_workflow.sh -S ~{black} -x ~{black} -r ~{r} -o . -M bincov_~{chr}_~{mode}.bed 2>&1 | tee cnmops.out
    RC=$?
    set -e
    if [ ! $RC -eq 0 ]; then
